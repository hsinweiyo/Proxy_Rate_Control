...
int proxy_func(int ser_port, int clifd, int down_rate, int up_rate);
int create_server(int port);
void down_rate_control(int byte, int rate);
void up_rate_control(int byte, int rate);
...
int main (int argc, char **argv) {
    ...
    for (;;) {
        ...
        if ((childpid = fork()) == 0) {
            ...
            */
            close(ctrlfd);
            proxy_func(FTP_PORT, connfd, down_rate, up_rate);
    ...
    for (;;) {
        ...
        if (nready > 0) {
            ...
            if (FD_ISSET(clifd, &rset)) {
                ...
                //flag++;
                //printf("Recursion(clifd): %d floor & byte_num : %d \n", flag, byte_num);
                /* Rate Control */
                data_sum += byte_num;
                ...
                up_rate_control(byte_num, up_rate);
            ...
            if (FD_ISSET(serfd, &rset)) {
                ...
                if (status == FTP_PASV_CODE && ser_port == FTP_PORT) {
                    ...
                    if ((childpid = fork()) == 0) {
                        ...
                        //flag--;
                        //printf("Recursion(serfd): %d floor\n", flag);

                        proxy_func(data_port, connfd, down_rate, up_rate);
                ...
                current_rate  = (1.0*data_sum) / (cur_moment - prev_moment);
                down_rate_control(byte_num, down_rate);
...
void down_rate_control(int byte_sum, int rate) {
    ...
    usleep(difference * (rate <= 150 ? 44 : rate <= 350 ? 42 : rate <= 400 ? 41 : 40));
}

void up_rate_control(int byte_sum, int rate){
    double current_cost, target_cost;
